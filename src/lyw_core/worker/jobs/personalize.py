...
async def personalize_concept(
    ...
    *,
    ...
    """Generate and persist a derived asset for a single concept.
    ...
    lesson_id:
        The LessonGraph id.
    concept_id:
        The ConceptNode id within that lesson.
    profile_id:
        The LearnerProfile id.
    kind:
        One of ``"relevel"``, ``"replace"``, or ``"mnemonic"``.
    ...
    if concept is None:
        raise ValueError(f"concept {concept_id!r} not found in lesson {lesson_id!r}")
