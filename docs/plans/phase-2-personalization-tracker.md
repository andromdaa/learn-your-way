...
- 2026-04-30 T11: MnemonicGenerator uses run_validators (raises ValidationError) rather than discard-on-fail. Rationale: a mnemonic is a single result with no fallback — discarding silently would leave the caller with nothing; surfacing the error lets the caller decide whether to retry or skip. Faithfulness check is span-boundary only; mnemonic text is a creative restatement and content-matching would incorrectly reject valid outputs.
- 2026-04-30 T10: EmphasisValidator fails only when BOTH conditions hold simultaneously (high-prereq concept with 0 items AND zero-prereq concept with ≥2 items). Rationale: requiring both conditions avoids false positives on small sections where zero-prereq concepts haven't yet been covered. ActiveLearningValidator treats bloom_level=None as 'remember' so untagged items (pre-T8 or non-MCQ paths) never silently satisfy the active learning gate.
