...
async def test_mind_map_job_writes_asset_and_saves_to_dao() -> None:
    ...
    call_kwargs = ctx["data_dir"].write_asset.call_args
    ...
    with (
        ...
        patch(
            ...
            side_effect=ValidationError(
                ["mind-map must have at least 2 nodes, found 1"]
            ),
        ),
    ):
        instance = mock_generator_cls.return_value
...
async def test_mind_map_lesson_not_found_raises() -> None:
    ...
    ctx = _make_ctx()
    ctx["db"].get_lesson_graph.return_value = None
    ...
    with patch(
        ...
        mock_generator_cls.return_value.generate.assert_not_called()
